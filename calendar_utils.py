...
def parse_event_datetime(date_str, time_str):
    """
    Parse event date and time into a datetime object using dateutil.parser
    
    Args:
        date_str: Date string (e.g., "November 15, 2024" or "2024-01-01")
        time_str: Time string (e.g., "7:00 PM" or "14:00")
    ...
    try:
        ...
        # Use dateutil's parser which handles a wide variety of formats
        event_date = dateutil.parser.parse(datetime_str)
    ...
    # Assume events last 2 hours if not specified
...
def generate_google_calendar_url(event_config):
    ...
    """
    start_date, end_date = parse_event_datetime(event_config['date'], event_config['time'])
